...
per_exp_test_un <- function(d, FUN) {
  d %>%
    ...
    mutate(p.adj = p.adjust(p.value, method = "BH")) %>%
    ungroup() %>%
    select(set, experiment, mean_dif = estimate, conf.low, conf.high, stat = statistic, p.value, p.adj)
}
...
aggr_test_un <- function(d, FUN) {
  d %>%
    ...
    mutate(fit = map(data, ~ FUN(value ~ condition, data = .x, paired = FALSE, conf.int = TRUE) %>% tidy())) %>%
    unnest(fit) %>%
    select(set,  W = estimate, conf.low, conf.high, stat = statistic, p.value)
}
```
...
g <- ggplot(un_t_test) +
  ...
  geom_point(aes(x = experiment, y = mean_dif)) +
  facet_wrap(~ set, scales = "free") +
